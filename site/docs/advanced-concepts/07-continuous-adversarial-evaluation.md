...
- **[ToolHazard](https://arxiv.org/abs/2608.11878)** (August 2026) takes "keep expanding the battery" and automates the expansion itself, an LLM pipeline synthesizes adversarial environments and injection payloads instead of a person hand-writing five variants, the way this chapter's lab does. The more interesting result isn't the benchmark itself, it's what happened when they trained a model on the adversarial trajectories it generated: attack success on Qwen3-8B dropped from 36.1% to 18.1%, benign task success went *up* at the same time, and the improvement carried over to a completely different benchmark the model never trained on. A battery doesn't just measure a defense, it can also generate the data that improves it.
