...
That optional metadata is also worth knowing about: most vector databases let you filter by it alongside the similarity search, for example "only search records where `topic` is `pets`." You won't need this yet, but it's a useful thing to know exists. Intermediate covers it in more depth.
