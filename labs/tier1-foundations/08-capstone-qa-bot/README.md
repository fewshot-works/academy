...
Companion lab for [Chapter 8: Capstone: A Q&A Bot Over Your Own Documents](https://fewshot-works.github.io/zero-to-agent/docs/tier-1-foundations/capstone-qa-bot). This is the Tier 1 capstone: a reusable bot that answers questions about a whole folder of documents, saved to disk so it doesn't have to redo the embedding work every time you run it.
...
1. **Move into the lab folder.** If you already cloned the repo for an earlier lab, just `cd` into this one:
   ...
   ```bash
   cd zero-to-agent/labs/tier1-foundations/08-capstone-qa-bot
   ```
