...
Companion lab for [Chapter 6: What Is RAG?](https://fewshot-works.github.io/zero-to-agent/docs/tier-1-foundations/what-is-rag). You'll build a tiny bot that answers a question using a made-up document as its only source of truth.
...
1. **Move into the lab folder.** If you already cloned the repo for an earlier lab, just `cd` into this one:
   ...
   ```bash
   cd zero-to-agent/labs/tier1-foundations/06-first-rag-bot
   ```
