...
**The judge is wrong twice, and that's the more important lesson.** Question 1's agent answer, "Fernwood Coffee Co. has 1 location," directly contradicts the reference answer of three, a factual error the judge should have caught, and instead passed. Question 2's agent answer never states the membership cost at all, and got passed anyway.

A 100% pass rate here doesn't mean the agent is flawless, it means `llama3.2` acting as judge is being too lenient, marking "close, but wrong" and "incomplete" as passing. This is the same limitation Intermediate Chapter 8 flagged: LLM-as-judge is a useful signal, not a ground truth, and a small local model judging another small local model's work compounds that unreliability. Don't trust a judge's verdict without spot-checking its reasoning against the actual answer, the way this README just did.
